<template>
    <div>
        ...
        <div class="row">
            <div class="col-12 col-md-8 post" v-for="post in posts" :key="post._id">
                <img class="post__picture" :src="post.user.picture" alt="profile picture" />
                <div class="post__body">
                    <div class="post__user">
                        <router-link
                            class="post__name"
                            :to="`/users/${post.user.sub.replace('auth0|', '')}`"
                    ...
                    <a
                        class="post__delete"
                        href="#"
                        v-if="post.user.sub === $auth.user.sub || $store.state.userRoles.includes('admin')"
                        @click="deletePost($event, post._id)"
                    >delete</a>
                    <a
                        class="post__update"
                        href="#"
                        v-if="post.user.sub === $auth.user.sub || $store.state.userRoles.includes('admin')"
                        @click="updatePost($event, post._id)"
                    >update</a>
                </div>
